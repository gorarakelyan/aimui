...
class PanelChart extends Component {
  ...
  drawAxes = (cb) => {
    ...
    const { xAlignment } = this.context.chart.settings.persistent;
    ...
    const xScale = d3.scaleLinear()
      .domain(this.context.chart.settings.persistent.zoom?.[this.props.index]?.x ?? [xMin, xMax])
  ...
  drawAggregatedLines = () => {
    ...
    const focusedLineAttr = focusedCircle?.runHash !== null ? focusedCircle : focusedMetric.runHash !== null ? focusedMetric : null;
    this.context.traceList?.traces.forEach(traceModel => {
      if (traceModel.chart !== this.props.index) {
        return;
      }

      const { run: runAvg, metric: metricAvg, trace: traceAvg } = traceModel.aggregation.avg;
      const { run: runMin, metric: metricMin, trace: traceMin } = traceModel.aggregation.min;
      ...
      this.lines.append('path')
        ...
        .attr('clip-path', 'url(#lines-rect-clip-' + this.props.index + ')')
        .style('fill', 'none')
        .style('stroke', this.context.traceList?.grouping?.color?.length > 0 ? traceModel.color : this.context.getMetricColor(runAvg, metricAvg, traceAvg))
        .style('stroke-dasharray', this.context.traceList?.grouping?.stroke?.length > 0 ? traceModel.stroke : '0')
        .attr('data-run-hash', runAvg.run_hash)
        .attr('data-metric-name', metricAvg.name)
        .attr('data-trace-context-hash', this.context.contextToHash(traceAvg.context))
        .on('click', function () {
          handleLineClick(d3.mouse(this));
        });
    });
  ...
  _renderPopUpContent = () => {
    ...
    return this.context.contextFilter.groupByChart.length === 0 || this.context.traceList?.traces.filter(trace => trace.chart === this.props.index).some(traceModel => traceModel.hasRun(run?.run_hash, metric?.name, this.context.contextToHash(trace?.context))) ? (
  ...
  _renderPanelMsg = (Elem) => {
    return (
      <div className='PanelChart__msg__wrapper'>
        {Elem}
      </div>
    );
  };
